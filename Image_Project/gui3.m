...
% Last Modified by GUIDE v2.5 12-Dec-2023 03:42:18
...
%%%i need to use the variable a

 global a 
 I_gray_image  = rgb2gray(a);
...
choice = questdlg('Do you want to exit the application?', ...
...
switch choice
    case 'Yes'
        ...
        close('all'); % Close the current figure (GUI)
    case 'No'
...
scalar = get(handles.scalar , 'string') ;
scalar1 = str2num(scalar) ;
...
%imag must to be grayScale

grayImg = rgb2gray(a);
image = histeq(grayImg);
...
histOfeqImage = imhist(image) ;
axes(handles.axes5);
imshow(histOfeqImage);
...
global a
% Define the filter kernel (e.g., 3x3 averaging filter)
...
img_noise = imnoise(a , "salt & pepper" , 0.02)

filterr = 1/16  * [  1,2,1  ;  2,4,2  ;  1,2,1 ] 

de_noise = filter2(filterr , img_noise)

axes(handles.axes5)

imshow(uint8(de_noise))
